...
<%
	...
	try {
		...
		Vector<String> vecCountries = usrData.getWazerConfig().getAuth().getActiveCountryCodes();
...
%>
			<tr class="DS-border-updn">
				...
				<td class="DS-padding-lfrg-4px DS-padding-top-4px DS-padding-bottom-0px" align="right">
					<span class="DS-cursor-pointer" title="Remove <%= countryName %> from <%= reqUser %> management rights" onClick="DeleteAuthCountry('<%= reqUser %>', '<%= usrCountry %>')">
		...
		<tr class="DS-border-up">
			<td class="DS-padding-updn-8px DS-cursor-pointer" ColSpan="4">
				<%= MdcTool.Button.TextIconOutlinedClass(
					...
					"onClick=\"AddAuthCountry('" + reqUser + "')\"",
					"Add New Country"
				) %>
			</td>
		</tr>
